        ...
        if unsafe { sys::RB_TYPE_P(self, sys::T_HASH) } {
            ...
            for i in 0..len {
                ...
                if let Err(error) = UncheckedValue::<K>::to_checked(key) {
                    return Err(CString::new(format!("Failed to convert key for HashMap<K, V>: {}", error.to_str().unwrap())).unwrap())
                }

                let val = unsafe { sys::rb_ary_entry(values, i) };
                if let Err(error) = UncheckedValue::<V>::to_checked(val) {
                    return Err(CString::new(format!("Failed to convert value for HashMap<K, V>: {}", error.to_str().unwrap())).unwrap())
                }
            }
            Ok(unsafe { CheckedValue::<HashMap<K, V>>::new(self) })
        } else {
            let val = unsafe { CheckedValue::<String>::new(sys::rb_inspect(self)) };
            Err(CString::new(format!("No implicit conversion of {} into HashMap<K, V>", val.to_rust())).unwrap())
        }
    }
}
